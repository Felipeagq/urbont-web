...
Sin Twilio, `/api/otp/send` devuelve el código en la respuesta **sólo fuera de
producción**; en producción responde 503.
...
3. **Variables de entorno** — *App settings → Environment variables*: cargar las
...
5. **Dominio** (opcional): *App settings → Domain management*.
...
En Supabase (*Authentication → URL Configuration*) hay que añadir la URL de
